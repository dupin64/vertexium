...
public class ElasticsearchSearchQueryBase extends QueryBase implements
        ...
        return new PagingIterable<Element>(getParameters().getSkip(), getParameters().getLimit(), pageSize) {
            ...
            protected ElasticsearchGraphQueryIterable<Element> getPageIterable(int skip, int limit, boolean includeAggregations) {
                ...
                for (SearchHit hit : hits) {
                    SearchHitField elementType = hit.getFields().get(Elasticsearch2SearchIndex.ELEMENT_TYPE_FIELD_NAME);
                    if (elementType == null) {
                        continue;
                    }
    ...
    protected TermQueryBuilder createElementTypeFilter(ElasticsearchElementType elementType) {
        return QueryBuilders.termQuery(Elasticsearch2SearchIndex.ELEMENT_TYPE_FIELD_NAME, elementType.getKey());
    }
